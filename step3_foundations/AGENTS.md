...
```bash
docker exec CONTAINER /usr/local/bin/pocketbase superuser upsert EMAIL PASS --dir /pb_data
```

⚠️ **PIÈGE DOCKER `--dir`** : Quand PocketBase tourne dans Docker, la CLI et le serveur
peuvent utiliser des data directories **différents**. Le serveur utilise le volume monté
(ex: `/pb_data` dans `docker-compose.yml`), mais la CLI défault à `/usr/local/bin/pb_data`.
Sans `--dir`, `superuser upsert` écrit dans une DB fantôme — le superuser semble créé
(message "Successfully saved") mais l'auth échoue systématiquement.
**Toujours** passer `--dir` correspondant au volume mount du `docker-compose.yml`.
...
### 6. Reset complet PocketBase

Quand PocketBase est dans un état incohérent (collections manquantes, auth qui échoue,
données corrompues), faire un reset complet :
...
docker compose up -d
sleep 2
# Puis recréer superuser (--dir !), collections, et seed data
```

### 7. Diagnostic PocketBase — Checklist

Si l'app ne trouve pas la DB ou l'auth échoue :
1. **Container tourne ?** → `docker ps | grep CONTAINER`
2. **API healthy ?** → `curl -s http://127.0.0.1:8090/api/health`
3. **Superuser dans la bonne DB ?** → Vérifier `--dir` dans la commande superuser
4. **Collection existe ?** → `curl -s http://127.0.0.1:8090/api/collections/NOM/records` (404 = collection manquante)
5. **Auth fonctionne ?** → Tester l'endpoint `_superusers/auth-with-password`
6. **Deux DB fantômes ?** → `docker exec CONTAINER find / -name "data.db"` — s'il y en a 2, c'est le piège `--dir`

### 8. Collection `events` — Schéma de référence
...
## Next.js 15+ — Pièges connus

- **`params` est un `Promise`** : Dans Next.js 15+, les `params` et `searchParams` des pages
  dynamiques sont des `Promise`. Accéder à `params.id` directement crashe silencieusement
  ou cause un rendu vide. Utiliser `React.use()` dans les Client Components :
  ```tsx
  ...
  ```
  Dans les Server Components, utiliser `await params` directement.

- **`useRouter`** : Toujours importer depuis `next/navigation` (PAS `next/router`).
...
| PocketBase Docker | CLI sans `--dir` | `--dir` = volume mount |
| Next.js params | `params.id` direct | `use(params)` ou `await params` |
